...
| 12.8 | Candle aberto não altera scores confirmados | ✅ (RC-007) | Teste `contrato 12.8`: o filtro `selectClosedCandles` exclui um candle em formação com valores extremos (spike 50%, volume 100x) e preserva o último candle fechado; fronteira `closeTime === asOf` incluída. A FIAÇÃO (todo caminho de score filtra antes de calcular) foi verificada por inspeção nas auditorias RC-001/RC-007 — `buildCoreAnalysis` e o motor v2 recebem apenas `closedCandles`. |

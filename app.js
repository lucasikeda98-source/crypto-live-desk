(function cryptoLiveDeskApp() {
  var MODEL_VERSION = '1.0.0-preview.7-codex.2';
  ...
  function buildRadarScore(symbol, analysis) {
    ...
    var fundamentalChecks = fundamentalContextChecks(ext);
    var parts = [
      ...
      { name: 'Fundamental', ruleId: 'radar.fundamental.v1', weight: 15, available: extEligible && fundamentalChecks.some(Boolean), value: clamp(((ext.asset + ext.defi) / 22) * 100, -100, 100), quality: fundamentalChecks.filter(Boolean).length / fundamentalChecks.length, raw: ext.asset + ext.defi, scope: 'symbol', reason: 'Variacoes 7d/30d de mercado e 1d/7d de TVL DeFiLlama' },
  ...
  function dataQuality(a) {
    ...
    var fundamentalChecks = [!!(a.coinMetrics && a.coinMetrics.latest && eligibleDataset(a.coinMetrics)) || nativeChainEligible, fundamentalContextChecks(currentExternal).some(Boolean)];

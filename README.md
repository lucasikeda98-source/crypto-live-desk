...
O workflow `.github/workflows/quality.yml` executa a suite deterministica em Node.js 22 (Ubuntu) e um smoke bloqueante no Edge headless (Windows) em cada pull request e push na `main`. O smoke recebe uma segunda tentativa para absorver oscilacao curta das fontes; duas falhas consecutivas bloqueiam o merge. Como a Binance pode restringir a regiao dos runners, a verificacao local e a validacao do deploy continuam obrigatorias antes de cada release.

...
### CC-FIX-05 — browser-boot vermelho no runner hospedado: filtro de ruido nao cobria o bloqueio CORS do geo-restrito (2026-07-17)

- Data: 2026-07-17
- Responsavel: Claude Code (Fable 5), por instrucao explicita do proprietario ("investiga a falha do browser-boot no CI" e depois "aplica a correcao e roda o gate completo")
- Sintoma: run 29591908037 (main `34f116f`) — `deterministic-tests` verdes (Node 22 e 24), `browser-boot` (BLOQUEANTE) exit 1 em 14s. Foi o PRIMEIRO run hospedado do filtro estrito do CC-FIX-03: os runs de `d385b2c` e `28417da` foram cancelados pela regra de concorrencia, entao o merge do preview.9 nunca teve CI hospedado verde.
- Causa raiz (diagnostico sistematico, reproducao antes do fix): no runner geo-restrito o `fapi.binance.com` responde 451 SEM `Access-Control-Allow-Origin`; o Chrome emite no console "Access to fetch at 'https://fapi.binance.com/...' has been blocked by CORS policy: ..." (manifestacao ja documentada na Fase B1). O filtro ANTIGO engolia essas mensagens por ACIDENTE (alternativa solta `fetch`); o CC-FIX-03 ancorou o filtro — corretamente contra falso-verde — mas nao incluiu nenhuma assinatura para o bloqueio de CORS. Resultado: ~10 mensagens vazam, o check "zero console.error proprios do app" reprova.
- Metodo de reproducao (registrado porque interceptacao ingenua da falso NEGATIVO): `route.fulfill` do Playwright NAO passa pela checagem de CORS do Chromium — simular o 451 por interceptacao PASSA indevidamente. A reproducao fiel exige CORS real: `--host-resolver-rules=MAP fapi.binance.com 127.0.0.1` + servidor HTTPS local (cert self-signed + `--ignore-certificate-errors`) respondendo 451 sem header CORS. Nessa condicao: 10 mensagens vazavam o filtro antigo do CC-FIX-03 -> exit 1 (bate com a annotation do CI, com a duracao de 14s do step e com o verde local). Evidencia adicional: o log do step e truncado "por tamanho" TAMBEM nos runs verdes (ruido normal do Chromium no runner) — tamanho de log nao e sintoma; raw logs exigem admin (403 anonimo).
- Correcao (TDD, teste primeiro em RED): assinatura ancorada `has been blocked by CORS policy` adicionada ao `networkNoise` (scripts/browser-boot-check.cjs) — frase gerada pelo NAVEGADOR na camada de rede, inalcancavel por console.error do app sem eco deliberado; preserva a intencao anti-falso-verde do CC-FIX-03 (as alternativas soltas `fetch`/`4\d\d`/`CORS` NAO voltaram).
- Teste de regressao: test/ops-behavior.test.js "CC-FIX-05" — extrai o regex DO PROPRIO script (nao uma copia), exige que as 4 assinaturas de rede do navegador (CORS-block real capturado, `net::ERR_FAILED`, `Failed to fetch`, WebSocket) sejam filtradas E que 4 erros tipicos do app (TypeError lendo 'fetch', ReferenceError, mensagem com 451, mensagem citando CORS de configuracao) continuem reprovando. Visto FALHAR antes do fix (na mensagem CORS) e passar depois.
- Gates executados apos a correcao (todos verdes):
  - `node --check` limpo nos 2 arquivos tocados.
  - `npm run test:coverage`: **346/346** (345 + o teste novo); cobertura **97,53 / 82,29 / 97,02** (pisos 95/75/90), exit 0.
  - `npm run test:boot`: **BOOT CHECK: OK** (13/13, com dados ao vivo e ciclo real de navegacao).
  - Reproducao geo-bloqueada re-executada com o filtro REAL do script: **0 vazamentos**, veredito "boot-check PASSARIA".
  - `npm run test:browser` (smoke autoritativo contra Binance real): **20/20**, zero excecoes, zero console errors.
- Escopo intocado: nenhuma regra de score, nenhum arquivo de lib/api/app; pin `4efe8ce2` e `rulesetVersion` inalterados. Diff total: 2 arquivos, +36/-1.
- Estado: correcao aplicada na arvore de trabalho, NAO commitada/publicada. Confirmacao final de ambiente vira do proprio CI no proximo push (unico ambiente com o geo-bloqueio real).

...
test('contrato 12.7: fallback equivalente recebe o fator de proveniencia registrado', () => {
  ...
  // O rotulo pode ser atribuido direto (source = '...') ou via ternario (ciclo D); capture
  // qualquer literal de string do arquivo que declare o fallback.
  const fallbackLabel = (marketApi.match(/'([^']*)'/g) || []).map((m) => m.slice(1, -1)).find((label) => /fallback/i.test(label));

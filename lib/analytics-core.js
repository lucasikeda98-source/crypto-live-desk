(function (root, factory) {
  ...
  function evaluateAlertTransitions(previous, current, config) {
    ...
    if (rules.scoreCross !== false && prevScore !== null && currScore !== null) {
      ...
      // O antigo -45 ("venda domina") era resquicio da era long-only: shorts alertavam num nivel
      // que nao correspondia a nenhuma decisao do painel.
      // Rotulos de ZONA, nao de decisao: a decisao "entrada com confirmacao/favoravel" depende de
      // gates (MTF, alinhamento, DC, vetos) que o alerta nao avalia — o alerta so ve o score cruzar.
      [{ level: 60, label: 'zona favoravel compradora' }, { level: 42, label: 'zona de confirmacao compradora' }, { level: -42, label: 'zona de confirmacao vendedora' }, { level: -60, label: 'zona favoravel vendedora' }].forEach(function (threshold) {

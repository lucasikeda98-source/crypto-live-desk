(function (root, factory) {
  ...
  function calculateCandleFlow(candles, cmfValue) {
    ...
    if (isFiniteNumber(lastVolume) && isFiniteNumber(averageVolume) && averageVolume > 0) {
      // High relative volume is participation, not direction — a high-volume selloff is not bullish,
      // so it no longer adds a directionless bonus. It still marks flow data as present; the
      // directional "volume x delta" read lives in the risk block (app.js).
      available = true;
    }
    if (cmf !== null) {
  ...
  function calculateCarryRegime(input) {
    ...
    // Symmetric tier magnitudes: euphoria (>15 / >30% a.a.) -> -2 / -3, mirrored by deep
    // backwardation (<-30% a.a. capitulation) -> +3. The +2 floor stays at -10% (not the -30 mirror
    // of the +15 tier) because the funding baseline is ~+11% a.a. (0.0001/8h annualized), so any
    // sustained negative funding is already anomalous — the threshold asymmetry is deliberate, the
    // magnitude asymmetry (the old missing +3 tier) was the bias.
    var carryScore = annualized > 30 ? -3 : annualized > 15 ? -2 : annualized < -30 ? 3 : annualized < -10 ? 2 : 0;
  ...
  /**
   * Wilson score interval (95% por padrao) para uma taxa de acerto observada. Comunica a
   * incerteza da amostra: 3 acertos em 4 sinais NAO e "75% de acerto", e "entre 30% e 95%".
   * Retorna { lower, upper } em porcentagem, ou null sem tentativas. Nao e calibracao de
   * probabilidade do modelo — apenas o intervalo estatistico da frequencia observada.
   */
  function wilsonInterval(successes, trials, zScore) {
    var n = toFiniteNumber(trials);
    var k = toFiniteNumber(successes);
    if (n === null || k === null || n <= 0 || k < 0 || k > n) return null;
    var z = numberOr(zScore, 1.96);
    var p = k / n;
    var z2 = z * z;
    ...
    return {
      lower: Math.max(0, (center - half) * 100),
      upper: Math.min(100, (center + half) * 100)
    };
  }
  ...
  function summarizeTradeJournal(records) {
    ...
    var cells = Object.keys(byKey).map(function (key) {
      ...
      return {
        ...
        hitRateInterval: wilsonInterval(cell.wins, cell.count),
  ...
  function calculateDerivativeDetailContribution(input) {
    ...
    var score = 0;
    // The absolute (annualized carry, from the caller) and relative (percentile, below) funding
    // lenses are complementary but tail-correlated: in euphoria both fire the same way. carryScore
    // is folded in here and the COMBINED funding-derived contribution is clamped to +/-7, so one
    // metric can't consume most of the +/-12 bucket while each lens keeps full authority alone.
    // carryScore defaults to 0, so callers that don't pass it keep the funding-only behavior.
    var carryScore = numberOr(input.carryScore, 0);
    ...
    if (isFiniteNumber(detail.fundingAvg) && isDatasetMetricEligible(detail, 'fundingAvg', asOf)) {
      ...
      fundingContribution = fundingPct !== null ? percentileExtremeContribution(fundingPct, 6, false)
    ...
    // Joint clamp of the two funding lenses: the percentile term above plus the annualized carry
    // from the caller, bounded to +/-7 so a single metric can't dominate the derivatives bucket.
    score += Math.max(-7, Math.min(7, fundingContribution + carryScore));
    return score;
  }
  ...
  /**
   * Model ruleset registry: weights, caps and thresholds for 1.0.0-preview.8
   * (merge do preview.6 da main com o preview.7-codex.2 do ciclo D).
  ...
  var RULESET = {
    ...
    rulesetVersion: '1.0.0-preview.8',
    ...
    // Contract 4.2/8.2/12.7: an EQUIVALENT fallback source (same metric/unit/window, e.g.
    // CoinPaprika replacing CoinGecko) stays eligible but earns partial Data Confidence credit.
    fallbackProvenanceFactor: 0.8
  };

  /**
   * Provenance factor for Data Confidence credit (contract 8.2/12.7): a dataset whose source
   * label declares an equivalent fallback (e.g. "CoinPaprika fallback") earns the registered
   * partial factor; native/primary sources (or absent labels) earn full credit. This grades
   * PROVENANCE only — eligibility and staleness are decided elsewhere.
   */
  function sourceProvenanceFactor(sourceLabel) {
    if (typeof sourceLabel !== 'string' || !sourceLabel) return 1;
    return /fallback/i.test(sourceLabel) ? RULESET.fallbackProvenanceFactor : 1;
  }
  ...
  /**
   * Aggregates journal hit-rates per score band; a hit is a positive 24h return for positive
   * scores and vice versa. Bands mirror the bidirectional decision ladder (+/-42, +/-60) so long
   * and short accuracy can be compared per band; the old single "<= -20" bucket hid the short side.
   */
  function summarizeSignalJournal(records) {
    ...
    return bands.map(function (band) {
      ...
      return {
        ...
        hitRateInterval: evaluated.length ? wilsonInterval(hits.length, evaluated.length) : null,
        median24h: evaluated.length ? median(evaluated.map(function (record) { return +record.outcome.r24h; })) : null,
        // Same minimum-sample bar as the trade journal: under 20 evaluations the hit rate is a
        // base-rate glimpse, not evidence.
        sufficient: evaluated.length >= 20
      };
    });
  }
  ...
  function evaluateAlertTransitions(previous, current, config) {
    ...
    if (rules.scoreCross !== false && prevScore !== null && currScore !== null) {
      // Espelha a ladder de decisao bidirecional do Ciclo C (+/-42 confirmacao, +/-60 favoravel).
      // Rotulos de ZONA, nao de decisao (RC-007): a decisao "entrada com confirmacao/favoravel"
      // depende de gates (MTF, alinhamento, DC, vetos) que o alerta nao avalia — o alerta so ve o
      // score cruzar. Os niveis vem do ruleset (ciclo D) para nao duplicar os literais.
      var zoneLabels = { '60': 'zona favoravel compradora', '42': 'zona de confirmacao compradora', '-42': 'zona de confirmacao vendedora', '-60': 'zona favoravel vendedora' };
      RULESET.alertScoreThresholds.forEach(function (level) {
        ...
        if (crossedUp || crossedDown) alerts.push({ id: 'score-' + level, message: current.symbol + ': Setup Score cruzou ' + (level > 0 ? '+' : '') + level + ' (' + (zoneLabels[String(level)] || 'zona') + ') em ' + currScore });
      });
    }
  ...
  var api = {
    ...
    sourceProvenanceFactor: sourceProvenanceFactor,

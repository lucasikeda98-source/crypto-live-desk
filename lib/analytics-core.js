(function (root, factory) {
  ...
  function calculateDerivativeDetailContribution(input) {
    ...
    // globalLongShortAccountRatio = varejo: contrarian leve (lotado = combustivel contra).
    if (isFiniteNumber(detail.longShortRatio) && isDatasetMetricEligible(detail, 'longShortRatio', asOf)) score += detail.longShortRatio > 1.7 ? -1 : detail.longShortRatio < 0.65 ? 1 : 0;
    // topLongShortPositionRatio = smart money: seguir (long positivo, short negativo).
    if (isFiniteNumber(detail.topPositionRatio) && isDatasetMetricEligible(detail, 'topPositionRatio', asOf)) score += detail.topPositionRatio > 1.8 ? 2 : detail.topPositionRatio < 0.65 ? -2 : 0;
  ...
  /**
   * Model ruleset registry: weights, caps and thresholds for 1.0.0-preview.3.
  ...
  var RULESET = {
    ...
    rulesetVersion: '1.0.0-preview.3',

...
local function validCurrentRecord(current)
  ...
  if current.id ~= incoming.id or current.id ~= field or current.inputSnapshotId ~= incoming.inputSnapshotId then return false end
  ...
  if type(current.rulesetHash) ~= 'string' or current.rulesetHash ~= incoming.rulesetHash or current.rulesetHash == '' or string.len(current.rulesetHash) > 128 then return false end

...
// Registros INCOMPLETOS tambem precisam de teto: cada um vira membro do indice global de
// vencimento (DUE_KEY), e o POST /api/signals nao e autenticado. Sem o cap, um cliente variando
// signalCloseTime cunha ids distintos sem limite, inflando o Redis e o backlog do worker.
const INCOMPLETE_CAP = 500;
const GLOBAL_RECORD_CAP = 10_000;
const COMPLETED_RETENTION_MS = 365 * DAY_MS;
const KEY_TTL_SECONDS = 400 * 24 * 60 * 60;
const DUE_KEY = 'cld:signals:due:v1';
const RECORD_INDEX_KEY = 'cld:signals:record-index:v1';
...
local globalRecordCap = tonumber(ARGV[7])
local staleIndexBefore = tonumber(ARGV[8])
local incoming = cjson.decode(incomingJson)
local currentJson = redis.call('HGET', key, field)
local merged = incoming

redis.call('ZREMRANGEBYSCORE', recordIndexKey, '-inf', staleIndexBefore)
if not currentJson and redis.call('ZCARD', recordIndexKey) >= globalRecordCap then
  return '__DURABLE_SIGNAL_CAPACITY__'
end
...
local function validCurrentRecord(current)
  ...
  if current.schemaVersion ~= 3 then return false end
...
redis.call('ZADD', recordIndexKey, asOf, member)
...
for _, field in ipairs(fields) do
  local member = namespaceHash .. '|' .. encodeURIComponent(field)
  redis.call('ZREM', dueKey, member)
  redis.call('ZREM', recordIndexKey, member)
end
redis.call('DEL', key)
return #fields
`;
...
local incompleteCap = tonumber(ARGV[5]) or completedCap
...
local function validRecord(record, field, serialized)
  ...
  if type(record.symbol) ~= 'string' or string.len(record.symbol) < 5 or string.len(record.symbol) > 19 or not string.match(record.symbol, '^[A-Z0-9]+USDT$') then return false end
...
for index = 1, #values, 2 do
  ...
  if not decoded or not validRecord(record, field, serialized) then
    ...
    elseif complete then
      ...
      table.insert(incomplete, { field = field, signalCloseTime = record.signalCloseTime, recordedAt = record.recordedAt })
    end
  end
end

local function byAge(left, right)
...
table.sort(completed, byAge)
for index = 1, math.max(0, #completed - completedCap) do removeField(completed[index].field) end
-- Incompletos tambem sao limitados (mais antigos primeiro): removeField limpa o membro do
-- indice global de vencimento, entao o DUE_KEY fica limitado por namespace junto com o hash.
table.sort(incomplete, byAge)
for index = 1, math.max(0, #incomplete - incompleteCap) do removeField(incomplete[index].field) end
...
function compactDurableSignals(records, asOf = Date.now()) {
  ...
  // Espelha o Lua de compactacao: incompletos tambem tem teto (mantidos os mais novos), para
  // que um cliente nao possa crescer o hash e o indice de vencimento sem limite.
  ...
  return complete.slice(Math.max(0, complete.length - COMPLETED_CAP))
    .concat(incomplete.slice(Math.max(0, incomplete.length - INCOMPLETE_CAP)))
    .sort((a, b) => a.signalCloseTime - b.signalCloseTime || a.recordedAt - b.recordedAt);
}

function createDurableSignalStore(redis, options = {}) {
  if (!redis) throw new TypeError('redis client is required');
  const globalRecordCap = Number.isFinite(options.globalRecordCap)
    ? Math.max(1, Math.floor(options.globalRecordCap))
    : GLOBAL_RECORD_CAP;
  ...
  async function upsertByHash(hash, records, asOf = Date.now(), retryDelayMs) {
    ...
    if (typeof redis.eval === 'function') {
      const argumentsFor = (incoming) => [
        ...
        String(globalRecordCap),
        String(asOf - KEY_TTL_SECONDS * 1000)
      ];
      if (typeof redis.pipeline === 'function') {
        const pipeline = redis.pipeline();
        unique.forEach((incoming) => pipeline.eval(ATOMIC_MERGE_AND_SCHEDULE_LUA, [key, DUE_KEY, RECORD_INDEX_KEY], argumentsFor(incoming)));
        const values = await pipeline.exec();
        if (values.includes('__DURABLE_SIGNAL_CAPACITY__')) {
          const error = new Error('Durable signal storage capacity reached');
          error.code = 'DURABLE_CAPACITY';
          throw error;
        }
      ...
      for (const incoming of unique) {
        ...
        await redis.zadd(RECORD_INDEX_KEY, { score: asOf, member: dueMember(hash, merged.id) });
    ...
    if (typeof redis.eval === 'function') {
      ...
      const values = await redis.eval(ATOMIC_COMPACT_LUA, [key, DUE_KEY, RECORD_INDEX_KEY], [
        hash,
        String(asOf),
        String(COMPLETED_RETENTION_MS),
        String(COMPLETED_CAP),
        String(INCOMPLETE_CAP)
      ]);
      return compactDurableSignals((Array.isArray(values) ? values : []).map((value) => parseStoredRecord(value, asOf)).filter(Boolean), asOf);
    }
    ...
    for (const record of removed) {
      ...
      await redis.zrem(RECORD_INDEX_KEY, dueMember(hash, record.id));
    }
    return all;
  }
  ...
  async function clear(namespace) {
    ...
    if (typeof redis.eval === 'function') {
      ...
      await redis.eval(ATOMIC_CLEAR_LUA, [hashKey(hash), DUE_KEY, RECORD_INDEX_KEY], [hash]);
      return;
    }
  ...
  async function due(asOf = Date.now(), limit = 20) {
    ...
    // REV-CC-02/G: membro orfao (registro sumiu do hash) sai TAMBEM do indice global de
    // capacidade — senao a entrada fantasma consome o teto de 10k ate a poda de 400 dias.
    await Promise.all(staleMembers.map((member) => Promise.all([
      redis.zrem(DUE_KEY, member),
      redis.zrem(RECORD_INDEX_KEY, member)
    ])));
    return output;
  }

...
function requestHost(request) {
  return String(header(request, 'x-forwarded-host') || header(request, 'host') || '').split(',')[0].trim().toLowerCase();
}
...
function clientKey(request) {
  // Vercel preserves its own client-IP header even when a proxy in front rewrites X-Forwarded-For.
  const forwarded = String(header(request, 'x-vercel-forwarded-for') || header(request, 'x-forwarded-for') || '').split(',')[0].trim();
  const raw = forwarded || request && request.socket && request.socket.remoteAddress || 'anonymous';
...
async function applyApiPolicyAsync(request, response, options = {}) {
  ...
  try {
    ...
    // availability without claiming that the global protection remained active.
    response.setHeader('X-RateLimit-Scope', 'instance-fallback');
    return true;
  }
}

function resetRateLimits() { buckets.clear(); lastBucketSweepAt = 0; distributedLimiter = null; distributedLimiterIdentity = ''; distributedLimiterRedis = null; }
function rateLimitBucketCount() { return buckets.size; }

module.exports = { applyApiPolicy, applyApiPolicyAsync, getDistributedLimiter, originAllowed, resetRateLimits, rateLimitBucketCount };

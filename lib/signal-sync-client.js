(function (root, factory) {
  ...
  function createClient(options) {
    ...
    async function request(method, records, journalId) {
    ...
    async function sync(records) {
      ...
      try {
        return await runExclusive(async function performSyncExclusive() {
          ...
          notify('syncing', null);
          var remote = await request('GET', undefined, journalId);
          if (superseded(generation)) return { ok: false, superseded: true };
          ...
          if (!merged.length) persisted = requireConfiguredPayload(await request('POST', [], journalId));
          for (var offset = 0; offset < merged.length; offset += batchSize) {
            persisted = requireConfiguredPayload(await request('POST', merged.slice(offset, offset + batchSize), journalId));
            if (superseded(generation)) return { ok: false, superseded: true };
          }
          if (superseded(generation)) return { ok: false, superseded: true };
    ...
    function clear() {
      ...
      state.clearPromise = (async function performClear() {
        try {
          ...
          try {
            await runExclusive(async function performClearExclusive() {
              ...
              requireConfiguredPayload(await request('DELETE', undefined, journalId));
              options.writeRecords([]);
            });
